...
commands["create"] = {
	...
	func = function(message, args)
		...
		if not sp or not cons then message:delete() message:reply("Invalid, or too few, fields: !create Name [stockpile(num)] [consumption(num)]") return end
...
commands["upkeepsummary"] = {
	...
	func = function(message, args)
		...
		if sent then
			BOT_STORAGE[message.guild.id].savedSummary = { channelId = message.channel.id, messageId = sent.id }
			saveData()
		end
		message:delete()
	end
...
commands["show"] = {
	...
	func = function(message, args)
		...
		if not target or target:lower() == 'all' then
			for _,e in pairs(BOT_STORAGE[message.guild.id].upkeepEntries) do util.addToDelete(message.channel:send(util.renderEmbed(e)),30) end
		else
			local found
			for _,e in pairs(BOT_STORAGE[message.guild.id].upkeepEntries) do if e.name:lower() == target:lower() then found = e; break end end
